...
fig.suptitle('Train images from the Paper', fontsize=16)
...
for img_name in os.listdir(test_img_folder):
    if not img_name.endswith('.ipynb_checkpoints'):
        image = mpimg.imread(test_img_folder+img_name)
        image_to_show = cv2.resize(image, (200, 200), interpolation = cv2.INTER_AREA)
        image = cv2.resize(image, (32, 32), interpolation = cv2.INTER_AREA)
        cnt = cnt+1
        #sub=plt.subplot(1,5,cnt)
...
n_classes = 1
y = np.zeros([test_images.shape[0],n_classes])
y[:,0] = 1
print(y[0])
X_test = test_images
y_test = y
...
plt.subplot(1,4,1)
#plt.imshow(np.reshape(X_test[0],[img_size,img_size]),cmap="binary")
#plt.imsave("x_test.png",np.reshape(X_test[0],[img_size,img_size]),cmap="binary")
plt.subplot(1,4,2)
#plt.imsave("x_pca.png",np.reshape(X_pca[0],[img_size,img_size]),cmap="binary")
plt.subplot(1,4,3)
#plt.imsave("x_adv.png",np.reshape(X_adv[0],[img_size,img_size]),cmap="binary")
plt.subplot(1,4,4)
#plt.imsave("x_adv_pca",np.reshape(X_adv_pca[0],[img_size,img_size]),cmap="binary")
#plt.show()
"""
################################################################################
"""
...
print("diff_clean",diff_clean)
print("diff adv",diff_adv)
...
print("PCA clean，",diff_clean,"  components =",pca_components)
print("PCA adv",diff_adv,"  components =",pca_components)
################################### reducing color bits #############################################
diff_clean_all = []
diff_adv_all = []
for bit_depth_1 in range(1,8):
    ...
    print("color reduce(bit_depth = {0})，".format(bit_depth_1),diff_clean)
    print("color reduce(bit_depth = {0})，".format(bit_depth_1),diff_adv)
# plt.figure()
...
diff_clean = com_diff(y_clean,y_redu_bit)
diff_adv = com_diff(y_adv,y_adv_redu_bit)
print("color reduce(bit_depth = {0})，".format(bit_depth),diff_clean)
print("color reduce(bit_depth = {0})，".format(bit_depth),diff_adv)
